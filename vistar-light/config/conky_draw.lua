require 'cairo'
package.path = package.path .. ';' .. os.getenv("HOME") .. '.config/conky/vistar-light/config/conky-draw/?.lua'
require 'conky_draw_config'


function rounded_rect(cr, x, y, w, h, radius)
    -- helper function to draw rounded rectangle
    cairo_new_sub_path(cr)
    cairo_arc(cr, x + w - radius, y + radius, radius, -90 * math.pi / 180, 0)
    cairo_arc(cr, x + w - radius, y + h - radius, radius, 0, 90 * math.pi / 180)
    cairo_arc(cr, x + radius, y + h - radius, radius, 90 * math.pi / 180, 180 * math.pi / 180)
    cairo_arc(cr, x + radius, y + radius, radius, 180 * math.pi / 180, 270 * math.pi / 180)
    cairo_close_path(cr)
end


function draw_frosted_glass_box(cr, x, y, w, h, radius)
    -- Frosted glass effect with multi-layer rendering
    
    -- Layer 1: Base semi-transparent dark rectangle (foundation)
    cairo_set_source_rgba(cr, 0.05, 0.05, 0.05, 0.35)
    rounded_rect(cr, x, y, w, h, radius)
    cairo_fill(cr)
    
    -- Layer 2: Multi-pass alpha overlays for depth (blur simulation)
    for i = 1, 8 do
        local alpha = 0.012 - (i * 0.001)  -- decreasing alpha
        cairo_set_source_rgba(cr, 1, 1, 1, alpha)
        rounded_rect(cr, x - i * 0.5, y - i * 0.5, w + i, h + i, radius + i * 0.3)
        cairo_fill(cr)
    end
    
    -- Layer 3: Inner glow/highlight (frosted effect)
    cairo_set_source_rgba(cr, 1, 1, 1, 0.08)
    rounded_rect(cr, x + 1, y + 1, w - 2, h - 2, radius - 1)
    cairo_fill(cr)
    
    -- Layer 4: Subtle noise pattern (texture simulation)
    math.randomseed(42)  -- consistent pattern
    for noise_i = 1, 100 do
        local px = x + math.random() * w
        local py = y + math.random() * h
        cairo_set_source_rgba(cr, 1, 1, 1, 0.02)
        cairo_rectangle(cr, px, py, 1, 1)
        cairo_fill(cr)
    end
    
    -- Layer 5: Top highlight edge (glass reflection)
    local gradient = cairo_pattern_create_linear(x, y, x, y + h * 0.3)
    cairo_pattern_add_color_stop_rgba(gradient, 0, 1, 1, 1, 0.15)
    ...
    -- Layer 6: Dark border (definition)
    cairo_set_source_rgba(cr, 0, 0, 0, 0.6)
    cairo_set_line_width(cr, 1.5)
    rounded_rect(cr, x, y, w, h, radius)
    cairo_stroke(cr)
    
    -- Layer 7: Outer subtle glow
...
function draw_box(display, element)
    -- Draw a simple colored box with optional border
    local cr = display
    ...
    -- Fill
    cairo_set_source_rgba(cr, hexa_to_rgb(element.color, element.alpha))
    rounded_rect(cr, x, y, w, h, radius)
    cairo_fill(cr)
    
    -- Border (if thickness > 0)
    if element.border_thickness > 0 then
        cairo_set_source_rgba(cr, hexa_to_rgb(element.border_color, element.border_alpha))
        cairo_set_line_width(cr, element.border_thickness)
        rounded_rect(cr, x, y, w, h, radius)
        cairo_stroke(cr)
    end
end
...
function check_network_interface(interface)
    -- Check if network interface is active by reading /proc/net/route
    ...
    -- Check if interface name appears in routing table
    return string.find(content, interface) ~= nil
end


-- Line graph for network speed (with historical data tracking)
-- Store historical data for each graph
local graph_history = {}
local graph_max_history = {}

function draw_line_graph(display, element)
    -- Only draw if the interface is active
    if not check_network_interface(element.interface) then
        return
    end
    ...
    -- Initialize history for this element if not exists
    local graph_id = element.interface .. '_' .. (element.conky_value_up or element.conky_value_down)
    if not graph_history[graph_id] then
        graph_history[graph_id] = {}
        graph_max_history[graph_id] = {}
    end
    
    -- Get current value
    local conky_value = element.conky_value_up or element.conky_value_down
    local current_value = get_conky_value(conky_value, true) or 0
    
    -- Add to history (keep last 150 points for 150px width)
    table.insert(graph_history[graph_id], current_value)
    if #graph_history[graph_id] > width then
        table.remove(graph_history[graph_id], 1)
    end
    
    -- Calculate dynamic max value (use recent peak with some headroom)
    local max_value = 100  -- minimum scale
    for i, value in ipairs(graph_history[graph_id]) do
        if value > max_value then
            max_value = value
        end
    end
    -- Add 20% headroom and smooth out sudden jumps
    max_value = max_value * 1.2
    table.insert(graph_max_history[graph_id], max_value)
    if #graph_max_history[graph_id] > 30 then
        table.remove(graph_max_history[graph_id], 1)
    end
    -- Use smoothed max from recent history
    local smoothed_max = 0
    for i, val in ipairs(graph_max_history[graph_id]) do
        smoothed_max = math.max(smoothed_max, val)
    end
    max_value = smoothed_max
    
    -- Draw background
    if element.background_alpha > 0 then
        cairo_set_source_rgba(cr, hexa_to_rgb(element.background_color, element.background_alpha))
        cairo_rectangle(cr, x, y, width, height)
        cairo_fill(cr)
    end
    
    -- Draw graph with Bezier smoothing
    if #graph_history[graph_id] > 2 then
        -- Convert values to coordinates
        local points = {}
        for i, value in ipairs(graph_history[graph_id]) do
            local px = x + (i - 1) * (width / math.max(#graph_history[graph_id] - 1, 1))
            local normalized_value = math.min(value / max_value, 1.0)
            local py = y + height - (normalized_value * height)
            table.insert(points, {x = px, y = py})
        end
        
        -- Draw filled area with Bezier curves
        cairo_set_source_rgba(cr, hexa_to_rgb(element.fill_color, element.fill_alpha))
        ...
        -- Draw the actual Bezier line on top
        cairo_set_source_rgba(cr, hexa_to_rgb(element.graph_color, element.graph_alpha))
        cairo_set_line_width(cr, element.graph_thickness)
...
requirements = {
    ...
    clock = {},
    box = {'from', 'width', 'height'},
    line_graph = {'from', 'width', 'height', 'interface'},
}
...
defaults = {
    ...
    line_graph = {
        ...
        graph_color = 0x00FF6E,
        graph_alpha = 0.7,
        graph_thickness = 1.5,
        fill_color = 0x00FF6E,
        fill_alpha = 0.3,
        draw_function = draw_line_graph,
    },
}
